...
func createPresetLanguageMessages(tx *gorm.DB) {
	...
	register.SetKey("acc_incorrect_token").Add("Incorrect token type", "令牌类型不正确", "令牌類型不正確")
	...
	register.SetKey("fano_table_search").Add("Search", "搜索", "搜索")
	...
	register.SetKey("kuu_i18n_value_required").Add("Value is required", "值不能为空", "值不能為空")
